...
```js
function ToPrimitive(target,type){ // 将目标对象转换为指定类型原始值[[primitive]]
...
计算 `val1 + val2`:
```js
if(typeof ToPrimitive(val1) == 'string' || typeof ToPrimitive(val2) == 'string' ) 
...
```
## 参考
[1.JS类型转换上](https://github.com/mqyqingfeng/Blog/issues/159)

[2.JS类型转换下](https://github.com/mqyqingfeng/Blog/issues/160)
