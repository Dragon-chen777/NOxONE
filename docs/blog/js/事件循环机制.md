...
栈Stack：`后进先出`，执行`同步`任务

队列Queue：`先进先出`，存放`异步`任务
...
微任务: `promise.then.catch.finally`，`queueMicrotask`，`process.nextTick`(仅在node.js中，且享有优先执行权)
...
[2.这一次，彻底弄懂JS执行机制](https://juejin.cn/post/6844903512845860872)
