...
## 1. Symbol
`Symbol`是ES6新增的`简单数据类型`，由Symbol构造函数生成，表示`独一无二`的值
...
s1 === s2 // false，独一无二
```
创建Symbol时，传入的参数会成为它的`description`属性
```js
...
```
## 3. Symbol属性遍历
Symbol属性只能通过`Object.getOwnPropertySymbols()`方法遍历，
...
`Symbol.for(str)`返回`description`为`str`的全局symbol变量，若没有则在全局注册一个
...
Singleton模式指的是调用一个类，任何时候返回的都是同一个实例，对于Node保证的是每次执行一个模块文件，返回的都是同一个实例，其核心思想是`将实例放到顶层对象global作为属性存在`
...
// a和b都应用同一个A类实例
```
问题在于，如果有个坏蛋在一个文件上修改了`global._a`，其他后面文件再引入`global._a`就GG了，那么可以用Symbol的唯一性和私用化属性这一特性了
```js
