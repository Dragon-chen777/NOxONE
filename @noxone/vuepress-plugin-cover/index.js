...
module.exports = (config, ctx) => ({
	define() {
		...
		for (let i = 0; i < NOXONE_COVER_CONFIG.main.length; i++) {
			// 优先级覆盖逻辑
		...
		compressFont(
			...
			'font.min.ttf',
			NOXONE_COVER_CONFIG.main.map((item) => item.content.join('')).join('') + NOXONE_COVER_CONFIG.nextTip + NOXONE_COVER_CONFIG.skipTip + 'NOxONE',
		)
